...
9. 分类漫画列表使用分类标题与排序请求分页；仅最后一个可见条目触发下一页，同一时刻最多一个下一页请求，并按漫画 ID 去重；离开页面会取消活动请求并以请求代次隔离迟到结果。
...
12. 漫画详情与章节使用两个独立资源状态并发加载；任一失败不清空另一资源，重试也只调用对应仓库方法；详情、章节和收藏操作分别持有可取消任务，页面离开后不接受迟到状态或收藏回调。
13. 章节仓库从第一页读取总页数，依次合并全部页面，按章节 ID 去重后整体反转，向详情层统一提供最新章节在前的完整列表；`Chapter.title` 与 `order` 保持服务端原始配对，详情行和 Reader header 共用同一元数据展示，不从 order 猜测或重写标题。
...
16. 阅读器通过滚动区域中的实际图片位置确定当前可见项，优先加载该项并只预取其后 2 张图片；图片下载并发独立限制为最多 3 个。每次索引加载都有独立代次，取消结果不展示为网络失败，旧任务迟到完成也不能清除重新进入可见窗口后启动的新任务；页面离开或内存压力会分别取消当前任务或清理解码缓存。
...
24. 已认证主导航以发现页为根；工具栏提供收藏与搜索入口，分类/搜索/收藏统一进入详情，详情章节继续进入阅读器，所有分支共享组合根提供的仓库与阅读器依赖。
