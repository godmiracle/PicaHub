...
- 增加漫画详情页，详情资料与章节列表并发独立加载并支持局部失败重试。
- 完成发现模块边界场景验收，覆盖分页、刷新保留、取消、空数据、局部失败和可选字段，并验证三条发现 UI 导航链路。
- 增加章节图片仓库，以最多 3 个并发请求读取全部分页，并提供确定顺序、ID 去重和整体取消。
- 增加通用两级图片管线，由 URLCache 保存原始响应、NSCache 按解码成本限制内存图片，并支持请求去重和单图重试。
